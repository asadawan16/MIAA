  ...
  return (
    <section className="bg-bg-deep pt-18 md:pt-22 pb-0">
      ...
      <motion.h1
        ...
        className="text-2xl sm:text-3xl md:text-4xl lg:text-[2.5rem] font-medium text-accent-cream tracking-tight leading-snug mb-6 md:mb-8 px-6 md:px-10 lg:px-16 mt-10 sm:mt-14 md:mt-20"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] items-stretch">
        {/* Left — image carousel */}
        <div className="relative overflow-hidden bg-primary h-[300px] sm:h-[400px] md:h-[500px] lg:h-[550px]">
          ...
          <div className="bg-primary p-6 md:p-10 flex flex-col justify-between">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] tracking-[0.2em] uppercase text-accent-wheat font-normal">
                  Featured
                </span>
              </div>
              <div
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="flex-1"
              >
                <p className="text-sm text-accent-wheat italic mb-3">
                  {article.date}, <span>by {article.author}</span>
                </p>

                <h2 className="text-xl sm:text-2xl md:text-3xl font-medium text-accent-cream tracking-tight leading-snug mb-3">
                  {article.title}
                </h2>

                <p className="text-base text-accent-cream leading-relaxed mb-6">
                  {article.description}
                </p>

                <Link
                  to={`/blog/${article.slug}`}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold tracking-wider uppercase text-accent-cream hover:text-accent-wheat transition-colors"
                >
                  Read More
            ...
            <div className="flex items-center gap-3 mt-8">
              <button
                onClick={prev}
                className="w-10 h-10 rounded-lg bg-secondary-terra text-white flex items-center justify-center hover:bg-secondary-rust transition-colors"
              >
                <ArrowLeft size={18} />
              </button>
              <button
